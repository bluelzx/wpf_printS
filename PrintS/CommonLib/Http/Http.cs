...
namespace CommonLib.Http
{
    ...
    public class Http
    {
        ...
        /// <returns></returns>  
        public static HttpWebResponse GetResponse(string url, int? timeout, string userAgent, CookieCollection cookies)
        {
        ...
        /// <param name="cookies">随同HTTP请求发送的Cookie信息，如果不需要身份验证可以为空</param>  
        /// <returns></returns>  
        public static HttpWebResponse PostResponse(string url, IDictionary<string, string> parameters, int? timeout, string userAgent, Encoding requestEncoding, CookieCollection cookies)
        {
            if (string.IsNullOrEmpty(url))
            {
                throw new ArgumentNullException("url");
            ...
            // url加随机数防止缓存，?rnd=" + Math.random();
            //Random random = new Random();
            //int rnd = random.Next(100, 999);
            //url += "?rnd=" + rnd.ToString();

            // url加时间戳
            url += "?rnd=" + DateTime.Now.ToString("yyyyMMddHHmmss");
